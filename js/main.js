...
for (var i = 0; i < objectsArr.length; i++) {
  var pinEl = renderPin(objectsArr[i]);
...
var renderCard = function (obj1) {
  ...
  let cardTextCapacity = cardElement.querySelector(`.popup__text--capacity`);
  let cardTextTime = cardElement.querySelector(`.popup__text--time`);
  let cardFeatures = cardElement.querySelectorAll(`.popup__feature`);
  ...
  for (let cardFeature of cardFeatures) {
    let cardClass = cardFeature.classList[1];
    let featureCode = cardClass.split(`--`)[1];
    let featureIndex = obj1.offer.features.indexOf(featureCode);
    if (featureIndex === -1) {
      cardFeature.remove();
    }
  }
  ...
  let cardPhotosChild = cardPhotos.querySelector(`.popup__photo`);
  ...
  if (obj1.offer.photos.length === 0) {
    ...
    for (i = 1; i < obj1.offer.photos.length; i++) {
      cardPhotosChild.src = obj1.offer.photos[0];
      let cardPhotoСlone = cardPhotosChild.cloneNode();
...
mapPins.addEventListener(`click`, function (evt) {
  ...
  if (pointer && !pointer.classList.contains(`map__pin--main`)) {
    ...
    if (!mapCard) {
      fragmentCard.appendChild(renderCard(objectsArr[pinAttr]));
